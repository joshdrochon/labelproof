...
def test_the_ci_run_makes_no_live_provider_call(tmp_path: Path) -> None:
    ...
    # Assembled rather than written out: `scripts/scan_secrets.py` blocks a commit
    # carrying a key-shaped literal, and it is right to. A tracked file that looks
    # like it holds a credential is how the scanner gets disabled by whoever hits it
    # next.
    fake_key = "-".join(  # noqa: FLY002 — the literal is what scan_secrets.py blocks
        ["sk", "ant", "not", "a", "real", "key"]
    )
    done = _run(["-c", probe], ANTHROPIC_API_KEY=fake_key)
    assert done.returncode == golden_exit(), done.stdout + done.stderr
...
def test_the_sweep_measures_accuracy_cost_and_latency_per_model() -> None:
    ...
    opus, haiku = results
    # Deliberately not `== 1.0`. `tc06_buried_warning` fails open until LP-211 lands,
    # so a literal would be wrong today — and a literal recomputed for today would go
    # red the moment LP-211 fixes it, which is the wrong direction for a test to fail
    # in. The property that must hold either way: the sweep clears the accuracy floor,
    # and every row it gets wrong is a gap somebody has already named.
    assert opus.report.accuracy >= ACCURACY_FLOOR
    wrong = {o.fixture for o in opus.report.outcomes if not o.correct}
    assert wrong <= KNOWN_LIVE_FALSE_PASSES, (
        f"unexplained failures: {sorted(wrong - KNOWN_LIVE_FALSE_PASSES)}"
    )
...
def test_every_warning_posture_reports_its_own_coverage() -> None:
    """Replaces a test that pinned a gap wave/warning has since closed.

    That test asserted `header_not_bold == 0` with the note "closed upstream — drop this
    test", and `tc03b_non_bold_warning_header` closed it. What is worth keeping is not
    the number but the property: every posture reports a real count, and the sweep can
    still name which ones are thin. Pinning today's counts would just be the same test
    again, red on the next fixture anyone adds.
    """
    coverage = sweep.posture_coverage(CATALOG)

    assert set(coverage) == set(sweep.WARNING_POSTURES)
    assert all(count >= 0 for count in coverage.values())
    assert coverage["header_not_bold"] >= 1, "tc03b closed this; something removed it"
    # The bar is six isolating renderings per posture (derived, not chosen — see
    # MIN_FIXTURES_PER_POSTURE). The set is nowhere near it, and the sweep must keep
    # saying so rather than blessing a model on this evidence.
    assert any(count < sweep.MIN_FIXTURES_PER_POSTURE for count in coverage.values())
