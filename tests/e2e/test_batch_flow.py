...
**These tests drive the app `create_app` builds, with nothing mounted here.** They used
to mount the batch router themselves, because `main.py` did not — which is exactly why
624 tests were green while `POST /batch` answered "that address is not part of this
tool". The helper that did it is gone now that `create_app` includes the router (before
`_install_spa`, so the catch-all does not shadow `GET /batch/{id}`) and calls
`install_verify_priority`. What this file proves and what ships are the same thing again.
...
def _client(tmp_path: Path, provider: Any, **overrides: Any) -> TestClient:
    """The shipped app, byte for byte. Nothing is mounted or reordered here.

    That is the point of this helper existing at all now: every request below goes
    through the same routing table a deployment has, so "the batch pipeline works" and
    "the batch endpoint is reachable" are one claim rather than two.
    """
    config = Config(
    ...
    return TestClient(create_app(config=config, provider=provider))
