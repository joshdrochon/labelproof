...
def test_a_buried_warning_is_read_rather_than_reported_as_absent(
    fixture_uploads: Any,
) -> None:
    """TC-06's first half: the statement is there and the tool says so.

    The warning is present, small, and low-contrast — legible to the extractor and
    invisible to a shopper. Reading it is not the same as it being prominent, and the
    two halves are asserted separately: this one says the *text* was read, the test
    below says the verdict is not a Match. Collapsing them would let a regression that
    reported the statement Missing look like the prominence rule doing its job.
    ...
    layer that exists to find bypasses. It is gone, and so is the marker.
    """
...
def test_a_buried_warning_is_flagged_for_prominence(fixture_uploads: Any) -> None:
    """CLOSED (LP-211). A present-but-buried warning no longer reaches the agent as a Match.

    This was `xfail(strict=True)`: prominence heuristics did not exist, so a statement
    printed visibly smaller and lower-contrast than the rest of the label scored a clean
    Match on its wording alone. 27 CFR 16.21 requires it be "conspicuous and prominent",
    and reading it off a photograph is not the same as it being readable on a shelf. The
    extractor already supplied `relative_size` and `contrast_ok`; the rule that consumes
    them landed and this turned red, which is the signal it was written to give.

    Left as the weakest true assertion — not-a-Match — rather than pinned to whichever
    verdict the prominence rule chooses. What the product promises here is that the row
    does not pass; which shade of not-passing it is belongs to the warning module's own
    tests, and asserting it from the E2E layer would break this on a rewording.
    """
    spec = by_name("tc06_buried_warning")
    client = _client(SpecBackedProvider(spec))
    body = _verify(
...
def test_an_unreadable_image_tells_the_agent_what_to_ask_for(
    ...
    # "this label", not "the label": Verify Now and the batch worker each carried their
    # own copy of this sentence in slightly different words until the pre-gate was moved
    # onto one shared path (`api.verify.prepare_images`). One sentence now, and it is the
    # batch wording that survived.
    assert "Nothing on this label could be checked" in rationale
