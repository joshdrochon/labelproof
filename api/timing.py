...
def _check_stage(name: str) -> None:
    """Reject anything that is not a directly measured stage."""
...
class RequestTimer:
    ...
    def record(self, name: str, duration_ms: int) -> None:
        """Add an already-measured duration to `name`.

        The other half of `stage()`, for work this timer cannot wrap. `prepare_images`
        is the shared pre-model path — one copy of ingest, quality scoring and the
        pre-gate, called by both Verify Now and Batch (LP-321) — and it returns the two
        durations it measured. Wrapping it in `stage()` instead would mean re-inlining
        the pipeline here to get a context manager around each phase, which is exactly
        the second copy that sharing it removed.

        Accumulates, like `stage()`, so a per-item loop adds up rather than overwriting.
        """
        _check_stage(name)
        # `or 0` rather than a cast: an unimplemented stage starts at None, and
        # timing it is exactly how it stops being unimplemented.
        setattr(self.timings, name, (stage_ms(self.timings, name) or 0) + duration_ms)
    ...
    def stage(self, name: str) -> Iterator[None]:
        ...
        try:
            ...
            self.record(name, _ms(self._clock() - started))
