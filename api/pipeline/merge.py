...
*But this is not the reconciliation the pipeline runs.* `api.verify` judges the warning row
on sightings, through `api.rules.warning.merge_sighting_typography`, which differs on
exactly one cell: `body_is_bold=True` on one photograph survives a contradiction from
another, because that value is a *sighting* of bold ink rather than a failure to see it,
and a violation the extractor detected must not be erased by a picture that missed it
(LP-217). Every other signal reconciles the same way in both places. `MergedLabel
.warning_typography` is still computed and still correct for a caller that wants the strict
form; it is simply not what decides the warning row, and wiring it back there would drop
the other branch's fix. Both rules are asserted, one in `tests/test_merge.py` and one in
