...
| **Ticket prefix** | `LP-001` … `LP-346` |
| **State** | **335 of 337 closed.** Derived from `Closes:` trailers by `scripts/sync_board.py` and a post-commit hook — never hand-edited. A close with no commit behind it is a false close. |
...
- [x] **LP-319** Fresh-eyes full walkthrough; final fixes (UX-1) — walked by Josh Rochon on 2026-08-14 against the deployed build. The findings became the producer punctuation fix, the keyboard-shortcut mnemonics, the masthead layout, the form alignment and the sample-card copy
