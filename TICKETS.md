...
| **State** | **331 of 335 closed.** Derived from `Closes:` trailers by `scripts/sync_board.py` and a post-commit hook — never hand-edited. A close with no commit behind it is a false close. |
...
- [ ] **LP-338** Glare alone may not refuse an image: `glare_score` counts near-saturated pixels, the generator paints paper at ~245 against BLOWN_LEVEL 250, and real scans clip to 255 — so two of 23 real white labels scored glare 0.000 with blur 1.000 and were refused unread, one of them the only label with no government warning (IMG-5, LP-321)
